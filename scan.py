...
from common import AV_STATUS_INFECTED_ALLOWED
from common import AV_STATUS_ERROR
from common import AV_SIGNATURE_UNKNOWN
from common import AV_STATUS_INFECTED_ALLOWED
from common import AV_STATUS_UNSCANABLE
from common import AV_SCANNED_MESSAGE
...
def set_av_tags(s3_client, s3_object, scan_result, scan_signature, timestamp, scan_message):
    ...
    for tag in curr_tags:
        if tag["Key"] in [
            ...
            if tag["Key"] == AV_STATUS_METADATA and tag["Value"] == AV_STATUS_INFECTED_ALLOWED:
                keep_status = True
            else:
                new_tags.remove(tag)
    ...
    new_tags.append({"Key": AV_SCANNED_MESSAGE, "Value": scan_message})
...
def lambda_handler(event, context):
    ...
    try:        
        ...
        scan_result, scan_signature, scan_message = clamav.scan_file(file_path)
        ...
        set_av_tags(s3_client, s3_object, scan_result, scan_signatue, result_time, scan_message)

        # Publish the scan results Un USE
